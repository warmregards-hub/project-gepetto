...
class CostTracker:
    ...
    async def log_cost(self, amount_usd: float, service: str, model: str, project_id: str, description: str):
        ...
        session_total, monthly_total, _ = await self.get_totals(project_id)
        if session_total > settings.cost_limit_session or monthly_total > settings.cost_limit_monthly:
            print("[Project Gepetto Cost] Limit exceeded, consider stopping execution.")

        return final_cost

    async def get_totals(self, project_id: str | None = None):
        ...
        try:
            ...
            return session_total, monthly_total, project_total
        except ProgrammingError as e:
            await self.db.rollback()
